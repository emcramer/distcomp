#' Create a worker object for use as a worker with master objects generated by [LogisticregressionMaster()]
#' @description `LogisticRegressionWorker` objects are worker objects at each site of
#' a distributed univariate linear model computation
#'
#' @docType class
#' @seealso `LogisticRegressionMaster` which goes hand-in-hand with this object
#' @importFrom R6 R6Class
#' @section Methods:
#' \describe{
#'   \item{`LogisticRegressionWorker$new(defn, data, stateful=TRUE)`}{Create a new LogisticRegressionWorker instance
...
LogisticRegressionWorker <- R6Class(
  "LogisticRegressionWorker",
  private = list(
    defn = NA 
    , stateful = FALSE
    , p = NA
    ##split the data into labels and features here?? -  leverage formula syntax?
    , data = NULL
    , data_features = NULL
    , data_labels = NULL
    , process1 = NULL
  ...
  public = list(
    initialize = function(defn, data, stateful=TRUE) {
      ...
      private$process1 <- as.matrix(model.matrix(formula, data=data))
      rownames(private$process1) <- NULL
      private$data_features <- private$process1
      private$process2 <- model.response(model)
      mat <- as.matrix(private$process2)
      rownames(mat) <- NULL
      private$data_labels <- mat
      private$data <- data
      private$p <- ncol(private$data_features)
    ...
    }, calcGrad = function(theta, ...) {
      m <- nrow(private$data_features)
      g <- e1071::sigmoid(private$data_features%*%theta)
      J <- (1/m)*sum((-private$data_labels*log(g)) - ((1-private$data_labels)*log(1-g)))
      return(J)
    }, kosher = function() {
      ## add sanity checks  
      TRUE
...
#' Create a master object to control worker objects generated by [LogisticRegressionWorker()]
#' @description `LogisticRegressionMaster` objects instantiate and run a distributed Cox model
#' computation fit
#' @docType class
#' @seealso `LogisticRegressionWorker` which generates objects matched to such a master object
#' @importFrom R6 R6Class
#' @section Methods:
#'
#' \describe{
#'   \item{`LogisticRegressionMaster$new(defn, data, debug=FALSE)`}{Create a new
#'         LogisticRegressionMaster object using the defn and data. The debug flag is useful for
#'         debugging}
...
#' @export
#' @format An [R6::R6Class()] generator object
LogisticRegressionMaster <- R6Class(
  ...
  public = list(
    initialize = function(defn, debug = FALSE) {
      ...
      ' Check for sanity'
      TRUE
      ##compute total cost amoung all sites
    }, sumGrad = function(theta) {
      debug <- private$debug
      if (debug) {
        print("theta")
        print(theta)
      ...
      n <- length(sites)
      if (private$dry_run) {
        mapFn <- function(x, arg) x$worker$calcGrad(arg)
      } else {
        mapFn <- private$mapFn
      }
      results <- Map(mapFn, sites, rep(list(theta), n))
      value <- Reduce(f = sum, results)
      if (debug) {
        print("value")
        print(value)
      }
      value # returns the sum of the cost for all sites
      ...
      ## TODO: update parameters using calculated gradient
      control <- list(iter.max=1000, eps=0)
      ### set first params to 0 for all, then update
      prevTheta = rep(0, p)
      fit <- optim(prevTheta 
                   ,fn = self$sumGrad
                   ,hessian = TRUE)
      ...
      # saving the result
      private$result <- result <- list(theta= fit$par
                                       , iter = fit$counts
    ...
    summary = function() {
      ...
      eCoef <- exp(result$theta)
      
      d <- as.data.frame(t(sapply(seq.int(length(result$theta)),
                                  function(i) {
                                    coef <- result$theta[i]
                                    eCoef <- exp(coef)
