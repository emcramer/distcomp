...
CoxMaster <- R6Class(
    ...
    public = list(
        initialize = function(defn, debug = FALSE) {
            ...
            sites <- private$sites
            n <- length(sites)
            # wraps the worker's log likelihood estimation function 
            if (private$dry_run) {
                mapFn <- function(x, arg) x$worker$logLik(arg)
            } else {
                mapFn <- private$mapFn
            }
            # the mapFn is a wrapper for calling the worker's log likelihood calculation
            # results contain the workers' log likelihoods given the set of passed beta estimations in a vector
            results <- Map(mapFn, sites, rep(list(beta), n))
            # the value contains the sum of the log likelihoods (scalar?), and assigns the name "value" for metadata
            value <- Reduce(f = sum, lapply(results, function(x, y) x[[y]], y = "value"))
            # the gradient contains the sum of the gradients from each worker to be used during Newton Raphson
            gradient <- Reduce(f = '+', lapply(results, function(x, y) x[[y]], y = "gradient"))
            # the hessian contains the sum of the hessian matrices (a matrix) generated by the 
            # maximum likelihood estimation of each worker
            hessian <- Reduce(f = '+', lapply(results, function(x, y) x[[y]], y = "hessian"))
            # the attributes of the return value are then set to contain the gradient and the hessian sums
            ...
            value # contains the sum of the log likelihoods with the gradient and hessian as "metadata"
