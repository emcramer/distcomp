...
PoissonRegressionWorker <- R6Class(
  ...
  public = list(
    initialize = function(defn, data, stateful=TRUE) {
      ...
      private$form <- as.formula(defn$formula)
      private$model <- model.frame(private$form, data=data)
      private$data <- list('X' = model.matrix(private$form, data=data)
                           , 'y' = model.response(private$model))
      private$p <- ncol(private$data$X)
      ...
      lambda <- exp(beta%*%t(private$data$X)) # calculate parameters
      ll <- -sum(dpois(private$data$y, lambda, log=TRUE)) # get negative log likelihood
      ll
    }, kosher = function() {
      ## add sanity checks (?)
      TRUE
